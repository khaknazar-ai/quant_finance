...
def remove_resume_section_from_builder(project_root: Path) -> None:
    """Remove the final resume/CV section from the notebook builder."""
    ...
    if start == -1:
        raise ValueError("Could not find markdown_cell start before resume marker.")

    # This section is the last notebook cell. Remove until the cells-list close.
    end = text.find("\n    ]", marker_index)
    if end == -1:
        raise ValueError("Could not find cells-list closing bracket after resume marker.")

    new_text = text[:start].rstrip()
    new_text += "\n    ]"
    new_text += text[end + len("\n    ]") :]

    if marker in new_text or "CV Bullets" in new_text:
        raise ValueError("Resume/CV section marker still present after removal.")

    path.write_text(new_text, encoding="utf-8")


def register_notebook_artifact(project_root: Path) -> None:
    """Register final research notebook in the artifact checker."""
...
def update_artifact_count_tests(project_root: Path) -> None:
    """Update expected artifact count from 31 to 32."""
...
def update_final_notebook_tests(project_root: Path) -> None:
    """Ensure tests reject resume/CV section in project notebook."""
    ...
    if 'assert "## CV Bullets" not in text' not in text:
        ...
        if anchor not in text:
            raise ValueError("Could not find notebook assertion anchor.")
        text = text.replace(
            anchor,
            anchor
...
def overwrite_notebook_artifact_tests(project_root: Path) -> None:
    """Write explicit tests for final notebook artifact registration."""
...
def main() -> None:
    project_root = Path(".")
    remove_resume_section_from_builder(project_root)
